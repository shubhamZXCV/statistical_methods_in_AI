...
class InternalDecisionNode(object):
    ...
    def predict(self, x_TF):
        ...
        # **START OF SOLUTION**
        # 1. Determine which examples go left and which go right
        # Examples go left if the feature value is LESS than the threshold
        left_mask_T = x_TF[:, self.feat_id] < self.thresh_val
        right_mask_T = np.logical_not(left_mask_T)

        # 2. Get predictions from the children
        # Slice x_TF to pass only the relevant examples to each child
        x_left = x_TF[left_mask_T]
        x_right = x_TF[right_mask_T]
        
        yhat_left = self.left_child.predict(x_left)
        yhat_right = self.right_child.predict(x_right)
        
        # 3. Aggregate all predictions and return one array
        # Initialize the result array
        yhat_T = np.empty(T, dtype=np.float64) 
        
        # Place the predictions back using the original masks
...
class LeafNode(object):
    ...
    # **START OF SOLUTION**
        # The prediction for a regression leaf node is the mean of its training labels
        mean_y = np.mean(self.y_N)
        
        # Return an array of size T where all elements are mean_y
        yhat_T = mean_y * np.ones(T)
        return yhat_T
        # **END OF SOLUTION**
