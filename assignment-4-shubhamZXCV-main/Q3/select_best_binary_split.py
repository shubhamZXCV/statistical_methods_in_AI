...
def select_best_binary_split(x_NF, y_N, MIN_SAMPLES_LEAF=1):
    ...
    for f in range(F):
        ...
        # **START OF SOLUTION**
        current_x_f = x_NF[:, f] # All feature values for current feature f

        for v in range(V):
            thresh_val = possib_xthresh_V[v]
            
            # Determine which examples go left and right
            left_mask_N = current_x_f < thresh_val
            right_mask_N = np.logical_not(left_mask_N)
            
            # Get the labels for the children
            y_L = y_N[left_mask_N]
            y_R = y_N[right_mask_N]
            
            # Calculate the prediction (mean) and cost (SSE) for the left child
            yhat_L = np.mean(y_L)
            cost_L = np.sum(np.square(y_L - yhat_L))
            
            # Calculate the prediction (mean) and cost (SSE) for the right child
            yhat_R = np.mean(y_R)
            cost_R = np.sum(np.square(y_R - yhat_R))
            
            # Store results for this threshold
            left_yhat_V[v] = yhat_L
            right_yhat_V[v] = yhat_R
            left_cost_V[v] = cost_L
            right_cost_V[v] = cost_R
